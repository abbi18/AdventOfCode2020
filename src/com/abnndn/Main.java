package com.abnndn;

public class Main {

    public static void main(String[] args) {
        // Twelth Day
        new com.abnndn.day12.First().calculateAnswer();
        new com.abnndn.day12.Second().calculateAnswer();

        // Elevnth Day
        new com.abnndn.day11.First().calculateAnswer();
        new com.abnndn.day11.Second().calculateAnswer();

        // Tenth Day
        new com.abnndn.day10.First().calculateAnswer();
        new com.abnndn.day10.Second().calculateAnswer();

        // Ninth Day
        new com.abnndn.day9.First().calculateAnswer();
        new com.abnndn.day9.Second().calculateAnswer();

        // Eighth Day
        // TODO: Better solution for second approach required
        new com.abnndn.day8.First().calculateAnswer();
        new com.abnndn.day8.Second().calculateAnswer();

        // Seventh Day
        new com.abnndn.day7.First().calculateAnswer();
        new com.abnndn.day7.Second().calculateAnswer();

        // Sixth Day
        com.abnndn.day6.First.calculateAnswer();
        com.abnndn.day6.Second.calculateAnswer();

        // Fifth Day
        com.abnndn.day5.First.calculateAnswer();
        com.abnndn.day5.Second.calculateAnswer();

        // Fourth Day
        com.abnndn.day4.First.calculateAnswer();
        com.abnndn.day4.Second.calculateAnswer();

        // Third Day
        com.abnndn.day3.First.calculateAnswer();
        com.abnndn.day3.Second.calculateAnswer();

        // Second Day
        com.abnndn.day2.First.calculateAnswer();
        com.abnndn.day2.Second.calculateAnswer();

        // First Day
        com.abnndn.day1.First.calculateAnswer();
        com.abnndn.day1.Second.calculateAnswer();
    }
}
